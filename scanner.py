...
def scanner(lines, tokenCode):
    ...
    for line in lines:
        ...
        # Si la línea no contiene ningun elemento entonces pasar a siguiente línea
        if len(items) <= 0:
            continue
        ...
        for item in items:
            ...
            instrumentPattern = re.compile(r"^[a-z]+$") # Una o más letras minúsculas
            isInstrument = bool(re.search(instrumentPattern, item)) # Buscar patrón en elemento

            # Checar si es una nota
            notePattern = re.compile(r"^((A|B|C|D|E|F|G)(#+|b+)?(-2|-1|0|1|2|3|4|5|6|7|8)|R)(w|h|q|e|s|t|f)(\.+|(t|3|5|7|9))?$")
            isNote = bool(re.search(notePattern, item)) # Buscar patrón en elemento

            # Checar si es una barra vertical
            barPattern = re.compile(r"^\|$") # Una barra vertical
            isBar = bool(re.search(barPattern, item)) # Buscar patrón en elemento
